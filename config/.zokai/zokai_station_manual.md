...
4. The backend automatically handles relationship mapping (e.g., converting `ALTERNATIVE_TO` to `COMPETES_WITH`) and entity deduplication — you **do not** need to manually fix non-standard relations
5. You review the extracted entities and relations for any glaring errors
6. Approved results are written to Postgres (entities, observations, relations)
7. Files are marked `indexed` with a SHA-256 content hash (used by `/refresh-all` to detect future changes)
...
>
> **Note on graph maturation**: As your Knowledge Graph grows, the "New Entities" count per batch will naturally decrease (because nodes for people/projects already exist), while the "Relations" count will scale as new facts are pinned to existing infrastructure.
...
| **Windows** | ZIP + PowerShell installer → `Zokai Station.bat` launcher in Documents folder → one-click launch |
...
| "Connect Google" button does nothing (Windows) | Persistent URL watcher died or was closed | Run `powershell -File core/scripts/open-url-watcher.ps1` to restore the background listener |
...
*Zokai Station User Manual — v1.2 — 2026-04-21*
