...
|---|---|
| **Dashboard** (`localhost:8081`) | Your command centre — live inbox and calendar at a glance. **Pro** adds a full task panel, system health indicators, a Kanban board, and an Ideas board. |
...
> 🎬 *See it in action: one prompt → a complete slide deck* — Calendar, tasks, and code pulled together without copy-pasting anything. *(Video walkthrough coming soon)*
...
3. Sends the batch to a dedicated lightweight LLM (`kw-extractor-mcp`) for entity + relation extraction
...
```
Pass 1 — /scan (free model, kw-extractor-mcp)
...
- Default free model (GLM-4.5 Air via OpenRouter) requires only a free OpenRouter key
...
- **Default (Free)**: GLM-4.5 Air via OpenRouter — free model, requires a free [OpenRouter key](https://openrouter.ai/keys) (no credit card)
- **Recommended**: Same OpenRouter key ($5 credit to start) → access to Claude, GPT-4o, Gemini, 500+ models
...
- Direct URL: `http://localhost:8081` (or the port shown in your `access.txt`)
...
> 🎬 *See it in action: AI curates a news digest* — your inbox summarised and organised by AI. *(Video walkthrough coming soon)*
...
> 🎬 *Watch this workflow* — AI audits a commission invoice, queries the database, catches €16,000 in phantom contracts. *(Video walkthrough coming soon)*
...
> 🎬 *Watch this workflow* — AI curates a news digest from Gmail. *(Video walkthrough coming soon)*
...
| **KW: Postgres not connected** | `kw-extractor-mcp` container not running or no DB secret | `docker compose -f core/docker-compose.yml -f docker-compose.pro.yml up -d kw-extractor-mcp` and check `docker logs zokai-mkn-kw-extractor-mcp` |
...
│  Nginx Reverse Proxy                                            │
│  Port: 8081 (Free) │ 8082 (Pro) │ 8080 (MKN)                  │
└────────────────────┬────────────────────────────────────────────┘
                     │
┌────────────────────▼────────────────────────────────────────────┐
...
| Service | Port |
|---|---|
| Dashboard / VS Code | 8081 (Free), 8082 (Pro), 8080 (MKN) |
| OAuth callback | 9001 (Free: 19001) |

**All source code** (except Pro-tier proprietary containers) is published under AGPL-3.0 at [github.com/Mkn501/Zokai-station-free](https://github.com/Mkn501/Zokai-station-free).

---

*Zokai Station User Manual — v1.2 — 2026-04-27*
