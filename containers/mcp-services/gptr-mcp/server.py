...
try:
    ...
    # Fallback if shared not mounted (e.g. dev)
...
def load_gptr_config():
    """Load model settings and Tavily API key from zokai-config.json (user-editable).

    Called per-request in deep_research() so user edits take effect immediately
    without container restart. Always overwrites os.environ (config is source of truth).
    PLACEHOLDER values are silently skipped — they never pollute os.environ.
    """
    config_path = os.getenv("ZOKAI_CONFIG_PATH", "/app/zokai-config.json")
    if not os.path.exists(config_path):
        return
    try:
        with open(config_path) as f:
            cfg = json.load(f)
        for key in ["FAST_LLM", "SMART_LLM", "RETRIEVER"]:
            val = cfg.get(key, "")
            if val:
                os.environ[key] = val
        # Inject Tavily key if user supplied a real key (not PLACEHOLDER)
        tavily_key = cfg.get("TAVILY_API_KEY", "")
        if tavily_key and tavily_key.upper() != "PLACEHOLDER":
            os.environ["TAVILY_API_KEY"] = tavily_key
            # Auto-upgrade retriever if not explicitly set in config
            if not cfg.get("RETRIEVER"):
                os.environ["RETRIEVER"] = "tavily"
    except Exception as e:
        logging.warning(f"Could not load zokai-config: {e}")


def load_secrets_from_files():
    """Load API keys from secret files defined in environment variables ending in _FILE"""
    for key in ["OPENAI_API_KEY", "TAVILY_API_KEY", "ANTHROPIC_API_KEY", "OPENROUTER_API_KEY"]:
        file_var = f"{key}_FILE"
        file_path = os.getenv(file_var)
        if file_path and not os.getenv(key):
            try:
...
# Load secret files at startup (stable baseline from Docker mounts)
load_secrets_from_files()
# NOTE: load_gptr_config() is called per-request in deep_research() so
# user edits to zokai-config.json take effect without container restart.
...
async def deep_research(query: str) -> Dict[str, Any]:
    ...
    """
    # Re-read zokai-config.json on every call so key changes take effect immediately
    load_gptr_config()
...
def run_server():
    ...
    # Auto-detect Docker environment ONLY if transport wasn't explicitly set to stdio
    ...
    try:
        ...
        # Note: mcp.run() blocks until the server is stopped
