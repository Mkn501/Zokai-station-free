...
▶ [AI generates a slide deck from a prompt](https://youtu.be/bqTI-GP8rkk) · [AI curates a news digest from your inbox](https://youtu.be/klV9WiVHHso) · [AI builds a commission controller app](https://youtu.be/tirmY4EfeqA)
...
That's it! Your workstation opens automatically at **http://localhost:8081** when ready.
...
> 🔑 **Password**: Your password is shown at the end of installation and saved to `access.txt` in your install folder. You'll need it every time you open the workstation.
...
3. Enter **any string** in the API key field (a placeholder is fine) and click **Save**
...
> 💡 Since the settings are pre-baked, you literally just need any string in the API key field to kick off local indexing.
...
|---|---|---|
| **Restart** | Double-click **Zokai Station Free.app** in your install folder | Double-click **install-free.bat** |
| **Stop** | `cd <install-folder>/workspace && docker compose -f docker-compose-free.yml down` | `docker compose --env-file .env.free down` |
...
All your data lives in your install folder — outside the repo, surviving upgrades.

> **Free → Pro**: Pro uses a separate data directory (`ZokaiData-pro/`) so both tiers can run side-by-side. The Pro installer will offer to migrate your notes and secrets.
...
- 🔍 **Tavily** (optional web search): [tavily.com](https://tavily.com)
